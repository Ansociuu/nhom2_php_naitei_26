<x-admin-layout title="Chỉnh sửa Tour: {{ $tour->title }}">
                ...
                <form method="POST" action="{{ route('admin.tours.update', $tour) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $tour->title) }}" required class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">
                            @error('title')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $tour->price) }}" step="1000" min="0" required class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">
                            @error('price')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="duration_days" id="duration_days" value="{{ old('duration_days', $tour->duration_days) }}" min="1" required class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">
                            @error('duration_days')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="departure_location" id="departure_location" value="{{ old('departure_location', $tour->departure_location) }}" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">{{ old('description', $tour->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="highlights" id="highlights" rows="3" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">{{ old('highlights', $tour->highlights) }}</textarea>
                        @error('highlights')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="included_services" id="included_services" rows="3" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">{{ old('included_services', $tour->included_services) }}</textarea>
                        </div>
                        <div>
                            <label for="excluded_services" class="block text-sm font-medium text-gray-700 mb-2">Dịch vụ KHÔNG BAO GỒM</label>
                            <textarea name="excluded_services" id="excluded_services" rows="3" class="w-full rounded-lg border-gray-300 focus:border-[#2D5A3D] focus:ring-[#2D5A3D] text-sm">{{ old('excluded_services', $tour->excluded_services) }}</textarea>
                        </div>
                    </div>
