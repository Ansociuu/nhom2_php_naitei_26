<x-guest-layout>
    <div class="px-7 pt-6 pb-8">
        <div class="grid grid-cols-2 gap-1 p-1 bg-gray-100 rounded-full">
            ...
            <a href="{{ route('register') }}" class="text-center py-2.5 text-sm font-bold rounded-full text-gray-500 hover:text-gray-800">
                Đăng ký
            </a>
        </div>

        <div class="mt-6 text-center">
            <h1 class="text-xl font-extrabold text-gray-900">Chào mừng trở lại 👋</h1>
            <p class="mt-1 text-sm text-gray-500">Đăng nhập để tiếp tục hành trình cuối tuần của bạn</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
                <label for="remember_me" class="inline-flex items-center">
                    <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-[#2D6A2D] shadow-sm focus:ring-[#2D6A2D]" name="remember">
                    <span class="ms-2 text-sm text-gray-600">Ghi nhớ đăng nhập</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-sm text-gray-500 hover:text-[#2D6A2D]" href="{{ route('password.request') }}">
                        Quên mật khẩu?
                    </a>
                @endif
            </div>
        ...
        <p class="mt-4 text-center text-sm text-gray-500">
            Chưa có tài khoản?
            <a href="{{ route('register') }}" class="font-semibold text-[#2D6A2D] hover:underline">Đăng ký ngay</a>
        </p>
    </div>
</x-guest-layout>
